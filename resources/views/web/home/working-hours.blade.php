                ...
                <div class="book-appintment position-relative p-4 br-2 bg-white">
                    <div class="row align-items-center justify-content-around">
                        ...
                        <div class="col-lg-3 col-md-6 text-center mb-lg-0 mb-3">
                            <select class="doctor-name-filter" aria-label="select doctor">
                                <option value="">{{ __('messages.web_home.select_doctor') }}</option>
                                @foreach($doctors as $doctor)
                                    <option
                                            value="{{ $doctor->id }}">{{ $doctor->doctorUser->full_name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-lg-3 col-md-6 text-center mb-lg-0">
                            <input class="form-control"  type="text" placeholder="{{ __('messages.web_appointment.select_time') }}"
                                   id="datepicker">
{{--                            <label><i class="ri-calendar-todo-line"></i></label>--}}
                        </div>
                        <div class="col-lg-3 text-end">
                            <a href="{{ route('appointment') }}" class="btn btn-primary d-block">
